...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Назначаем обработчик для всех кнопок уровней (1–20)
    ...
    // Обработчик нажатий на кнопки уровней
    override fun onClick(v: View?) {
        if (v is Button) {
            val text = v.text.toString()
            val levelNumber = text.toIntOrNull() ?: 0
            // Обновляем TextView через ресурсы
            val message = getString(R.string.info_clicked, levelNumber)
    ...
    private fun addDynamicButton() {
        val newButton = Button(this).apply {
            ...
            setOnClickListener {
                // При нажатии на динамическую кнопку тоже показываем информацию
                infoTextView.text = (it as Button).text
                Log.d(TAG, "Нажата динамическая кнопка: ${it.text}")
            }
            // Регистрируем контекстное меню для этой кнопки
    ...
    override fun onCreateContextMenu(menu: ContextMenu?, v: View?, menuInfo: ContextMenu.ContextMenuInfo?) {
        ...
        if (v is Button) {
            contextButton = v // сохраняем для последующей обработки
